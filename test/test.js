      ...
      it('it should POST and create a new key', (done) => {
        chai.request(app)
            ...
            .end((err, res) => {
                  ...
                  expect(resBody.skey).toBe("demo1")
                  expect(resBody.svalue).toBe("Value2")
              done();
            });
      });
  });
    ...
    it('it should GET a key previously created', (done) => {
      chai.request(app)
          ...
          .end((err, res) => {
                ...
                res.body.should.be.a('object');
                let resBody = res.body;
                expect(resBody.key).toBe("demo1")
                expect(resBody.value).toBe("Value2")
            done();
          });
    });
});
...
describe('/GET a Key with timestamp', () => {
    it('it should GET a key previously created when timestamp is passed', (done) => {
      ...
      chai.request(app)
          .get('/api/v1/object/demo1?timestamp='+currentTime)
          .end((err, res) => {
                res.should.have.status(200);
                res.body.should.be.a('object');
                let resBody = res.body;
